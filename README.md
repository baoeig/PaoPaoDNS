...
ROUTE_MODE|`cn_first`|`cn_first`,`foreign_first`,`gfwlist`|
...
QUERY_LOG_MAX_MB|`10`|`1-1024`，单位 MB|
QUERY_LOG_CLEAN_INTERVAL|`600`|`60-86400`，单位秒|
QUERY_ANSWER_LOG_MAX_LINES|`5000`|`100-100000`|
...
- ROUTE_MODE：仅在CNAUTO=yes时生效，默认值为`cn_first`。`cn_first`为原始逻辑，先按国内解析/IP库判断；`foreign_first`先匹配预分类的国外域名规则，未命中一律按国内处理；`gfwlist`先匹配[gfwlist](https://github.com/gfwlist/gfwlist)，未命中一律按国内处理。       
...
- QUERY_LOG_MAX_MB：管理后台查询日志 `/tmp/mosdns.log` 的最大容量，单位 MB。定时检查发现超限后保留最新约 75% 内容。
- QUERY_LOG_CLEAN_INTERVAL：查询日志定时检查间隔，单位秒。管理后台保存后无需重启后台服务，最长约一分钟应用新值。
- QUERY_ANSWER_LOG_MAX_LINES：持久化查询响应摘要 `/data/query_answer_log.jsonl` 最多保留的行数。

### ROUTE_MODE 分流模式使用
`ROUTE_MODE`用于选择自动分流的判断模型，仅在`CNAUTO=yes`时生效。强制列表始终优先于自动分流，顺序为：`custom_mod seq: top` > `force_forward_list` > `force_dnscrypt_list` > `force_recurse_list` > `force_ttl_rules` > `custom_mod seq: list` > `ROUTE_MODE`自动分流。

三种模式区别：
- `cn_first`：默认原始模式。先尝试国内解析，再结合CN域名/IP判断；未识别为国内时再进入非CN链路。适合继续使用原有行为。
- `foreign_first`：先匹配预分类的国外域名数据库，命中后走非CN链路；未命中一律按国内处理。适合希望“非国外名单都当国内”的场景，但依赖预分类数据覆盖度。
- `gfwlist`：先匹配[gfwlist/gfwlist](https://github.com/gfwlist/gfwlist)，命中后走非CN链路；未命中一律按国内处理。适合只想让gfwlist内域名走非CN，其余域名尽量按国内处理的场景。

使用管理后台修改：打开管理后台，在“分流模式”中选择`gfwlist`，保存后点击“重载mosdns”。首次启用会自动下载`/data/gfwlist.txt`，并转换为运行时使用的`/tmp/gfwlist.txt`。

使用`custom_env.ini`修改：
```ini
ROUTE_MODE="gfwlist"
```
保存后会触发mosdns重载；如果没有自动重载，也可以在管理后台点击“重载mosdns”，或执行：
```sh
docker exec <容器名> /usr/sbin/regen_mosdns.sh
```

配合`AUTO_FORWARD`时：
- `AUTO_FORWARD=yes`且`CUSTOM_FORWARD=IP:PORT`有效：`gfwlist`命中的域名会转发到`CUSTOM_FORWARD`，常用于fake-ip网关或代理DNS。
- `AUTO_FORWARD=no`：`gfwlist`命中的域名会走`force_dnscrypt_list`同一条加密DNS链路。
- 未命中`gfwlist`的域名会走国内默认链路，不会因为`USE_MARK_DATA`里的国外标记再被自动转到非CN。

验证方式：
```sh
# 查看gfwlist是否已生成
docker exec <容器名> sh -lc 'wc -l /tmp/gfwlist.txt; grep -Fx "domain:google.com" /tmp/gfwlist.txt'

# 查询测试，端口按自己的映射修改
dig +time=3 +tries=1 @127.0.0.1 -p 53 google.com A
dig +time=3 +tries=1 @127.0.0.1 -p 53 baidu.com A
```
开启管理后台查询日志后，典型结果为：`google.com`命中`gfwlist`后显示`CUSTOM_FORWARD`或加密DNS链路；`baidu.com`、未在gfwlist内的自定义域名显示`gfwlist -> 国内本地递归 5301`或国内回退链路。
...
- `Country.mmdb`：GeoIP数据库（完整版，含所有国家），自动更新将会覆盖此文件。容器版本更新将会覆盖该文件。  
...
- `gfwlist.txt`：`ROUTE_MODE=gfwlist`时出现，保存从[gfwlist/gfwlist](https://github.com/gfwlist/gfwlist)下载的原始规则，运行时会转换为 mosdns 可用的域名列表。修改将实时重载生效。容器版本更新不会覆盖该文件。   
